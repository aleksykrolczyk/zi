...
class LIFO_Authority_Policy:
    ...
    def getURL(self, c, iteration):
        ...
        if not c.authority:
            while next in self.fetched:
                if not self.queue:
                    return
                next = self.get_next(c)
            self.fetched.add(next)
    ...
    def get_next(self, container):
        if not container.authority:
            return self.queue.pop()
        values = np.array(list(container.authority.values()))
        probabilities = values / np.sum(values)
        return np.random.choice(list(container.authority.keys()), p=probabilities)
...
def calculate_authority(container):
    result = defaultdict(lambda: 1)
    totalPath = "./" + container.example + "/incoming_urls/incoming_urls.txt"
    with open(totalPath, 'r') as file:
        for line in file:
            target, _ = line.split(' ')
            result[target] += 1
    container.authority = result
